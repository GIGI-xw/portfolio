...
(function () {
  ...
  /* =============================
     6.6 一键选择本地作品图（双击 / 长按 / 上传按钮）
     支持：直接替换轮播里的占位图，支持 localStorage 记住
     ============================= */
  (function imageUploader() {
    ...
    var currentSlot = null; // 当前要替换的 slide 元素
    ...
    fileInput.addEventListener("change", function () {
      ...
      reader.onload = function (ev) {
        ...
        applyImage(currentSlot, url, file.name);
        // 尝试存到 localStorage（小图能存，大图会失败，失败就忽略）
        try {
          var key = storageKey(currentSlot);
          localStorage.setItem(key, url);
          localStorage.setItem(key + "__name", file.name);
        } catch (e) {}
      };
    ...
    function storageKey(slot) {
      var project = slot.closest(".project");
      var slotNum = project ? project.getAttribute("data-slot") : "x";
      var track = slot.closest("[data-carousel-track]");
      var idx = track ? Array.prototype.indexOf.call(track.children, slot) : 0;
      return "portfolio__img__" + slotNum + "_" + idx;
    }

    function applyImage(slot, url, name) {
      slot.classList.remove("media__placeholder");
      slot.innerHTML = "";
      ...
      wrap.className = "media__img-actions";
      var repl = document.createElement("button");
      repl.className = "media__img-btn";
      repl.type = "button";
      repl.textContent = "↻ 换图";
      repl.setAttribute("aria-label", "更换图片");
      on(repl, "click", function (e) {
        e.preventDefault();
        e.stopPropagation();
        currentSlot = slot;
        fileInput.click();
      ...
      clear.className = "media__img-btn media__img-btn--danger";
      clear.type = "button";
      clear.textContent = "✕ 清除";
      clear.setAttribute("aria-label", "清除图片");
      on(clear, "click", function (e) {
        e.preventDefault();
        e.stopPropagation();
        slot.classList.add("media__placeholder");
        var project = slot.closest(".project");
        var slotNum = project ? project.getAttribute("data-slot") : "";
        var track = slot.closest("[data-carousel-track]");
        var idx = track ? Array.prototype.indexOf.call(track.children, slot) + 1 : 1;
        var total = track ? track.children.length : 3;
        slot.innerHTML =
          '<span class="media__num">' + slotNum + '</span>' +
          '<span class="media__hint">图片 ' + idx + '/' + total + ' · 待填写</span>';
        attach(slot);
        try { localStorage.removeItem(storageKey(slot)); localStorage.removeItem(storageKey(slot) + "__name"); } catch (e) {}
      });
    ...
    function attach(slot) {
      ...
      on(slot, "touchend touchmove touchcancel", function () {
        if (timer) { clearTimeout(timer); timer = null; }
      }, { passive: true });

      // 占位时显示『点击选图』提示按钮
      if (slot.classList.contains("media__placeholder") && !$(".media__upload-btn", slot)) {
        var btn = document.createElement("button");
        btn.className = "media__upload-btn";
        btn.type = "button";
        btn.innerHTML =
          ...
          '<span>点击选择作品图</span>';
        btn.setAttribute("aria-label", "选择作品图片");
    ...
    // 初始化所有轮播 slide
    $$("[data-carousel] .carousel__slide").forEach(function (slot) {
      // 尝试从 localStorage 恢复已选图
      try {
        var key = storageKey(slot);
        var saved = localStorage.getItem(key);
        var savedName = localStorage.getItem(key + "__name");
        if (saved) {
          applyImage(slot, saved, savedName);
          return;
        }
      } catch (e) {}
      attach(slot);
